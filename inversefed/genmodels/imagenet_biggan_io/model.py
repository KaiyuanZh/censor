...
class Generator(nn.Module):
    ...
    def forward(self, z, ys, truncation, start_layer, end_layer):
        # print("start_year:{}".format(start_year))
        # print("The number of layer is:{}".format(len(self.layers)))
        if start_layer == 0:
            cond_vector = torch.cat((z, ys), dim = 1)
            h = self.gen_z(cond_vector)

            # We use this conversion step to be able to use TF weights:
            # TF convention on shape is [batch, height, width, channels]
            # PT convention on shape is [batch, channels, height, width]
            h = h.view(-1, 4, 4, 16 * self.config.channel_width)
            ...
            cond_vector = ys
        # print("h:{}".format(h))
        # print("cond_vector:{}".format(cond_vector))

        for i, layer in enumerate(self.layers[start_layer:end_layer]):
            if isinstance(layer, GenBlock):
                h = layer(h, cond_vector, truncation)
            else:
                h = layer(h)
        # print("h:{}".format(h))
        # print("cond_vector:{}".format(cond_vector))
        ...
        #     print("Size of h:{}".format(h.shape))
        #The last layer
        if end_layer == len(self.layers):
            # print("The length of layer:{}".format(i))
            h = self.bn(h, truncation)
            h = self.relu(h)
            h = self.conv_to_rgb(h)
            h = h[:, :3, ...]
            h = self.tanh(h)
            # print("The h's shape:{}".format(h.shape))
            return h, cond_vector
        else:
            # return h, cond_vector             #edit
            # print(f"Present end_layer:{end_layer}")
            # print(f"send h's shape:{h.shape} cond_vector's shape:{cond_vector.shape}")
            return h, cond_vector     #edit
...
class BigGAN(nn.Module):
    ...
    def forward(self, z, y, truncation):
        ...
        w, ys = self.generator(z, embed, truncation, self.start_layer, self.end_layer)
        return w, ys
